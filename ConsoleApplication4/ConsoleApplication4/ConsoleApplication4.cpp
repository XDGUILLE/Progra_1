...
void mostrarPromedioNotas(Alumno(&alumnos)[3]) {
	for (int i = 0; i < 3; ++i) {
		std::cout << "Alumno: " << alumnos[i].nombreAlumno << std::endl;
		for (int j = 0; j < 2; ++j) {
			std::cout << "  Curso: " << alumnos[i].cursos[j] << std::endl;
			std::cout << "  Notas: ";
			for (int k = 0; k < 3; ++k) {
				std::cout << "-Parcial " << k + 1 << ": ";
				std::cout << alumnos[i].notas[j][k] << " ";
			}
			std::cout << std::endl;
		}
		std::cout << std::endl; // Línea en blanco entre alumnos
	}
}
...
void mostrarPromedio() {
	for (int x = 0; x <= 2; x++) {
		for (int y = 0; y <= 1; y++) {
			cout << "El promedio del alumno " << x << " del curso " << y <<
